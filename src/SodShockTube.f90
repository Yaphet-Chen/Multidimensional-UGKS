...
module ControlParameters
    ...
    real(KREAL), parameter                              :: MAX_TIME = 0.14 !Maximal simulation time
    ...
    !Output control
    character(len=12), parameter                        :: HSTFILENAME = "SodShockTube" !History file name
    character(len=12), parameter                        :: RSTFILENAME = "SodShockTube" !Result file name
    character(len=12), parameter                        :: DISFILENAME = "SodShockTube" !Distribution file name
    ...
    real(KREAL), parameter                              :: KN = 0.0001 !Knudsen number in reference state
    ...
    !Geometry
    real(KREAL), parameter                              :: START_POINT = 0.0, END_POINT = 1.0
    ...
    real(KREAL), parameter, dimension(3)                :: CONVARS_LEFT = [1.0,0.0,1.5]
    real(KREAL), parameter, dimension(3)                :: CONVARS_RIGHT = [0.125,0.0,0.15]

    !--------------------------------------------------
    !Discrete velocity space
    !--------------------------------------------------
    integer(KINT)                                       :: uNum = 101 !Number of points in velocity space
    real(KREAL), parameter                              :: U_MIN = -6.0, U_MAX = +6.0 !Minimum and maximum micro velocity
...
contains
    ...
    subroutine Boundary()
        ...
        ctr(IXMIN-GHOST_NUM)%conVars = ctr(IXMIN)%conVars
        ctr(IXMAX+GHOST_NUM)%conVars = ctr(IXMAX)%conVars
        call DiscreteMaxwell(ctr(IXMIN-GHOST_NUM)%h,ctr(IXMIN-GHOST_NUM)%b,GetPrimary(ctr(IXMIN)%conVars))
        call DiscreteMaxwell(ctr(IXMAX+GHOST_NUM)%h,ctr(IXMAX+GHOST_NUM)%b,GetPrimary(ctr(IXMAX)%conVars))
...
contains
    ...
    subroutine Init()  
        ...
        call InitFlowField(GetPrimary(CONVARS_LEFT),GetPrimary(CONVARS_RIGHT)) !Set the initial value
    end subroutine Init
